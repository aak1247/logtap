...
为了满足不同规模业务的需求，`logtap` 在存储引擎选型上规划了清晰的分层架构：
...
| 核心指标维度 | 开源版 (PostgreSQL / TimescaleDB) | 企业版 / 云端版 (ClickHouse 引擎) | 业务适配选型建议 |
|---|---|---|---|
| **接入峰值 EPS** | 10,000 ~ 20,000 EPS | **100,000 ~ 300,000+ EPS** | 中小规模自建 vs 海量日志中心 |
| **持续稳定落盘** | ~10,000 EPS（受单机行存及倒排索引随机IO限制） | **100,000+ EPS（LSM-Tree 列存批量流式写入）** | 准实时审计 vs 高并发高吞吐流水 |
| **存储数据压缩比** | 约 1 : 1.5 ~ 1 : 2（行式与B树索引） | **1 : 5 ~ 1 : 10（列式 ZSTD/LZ4 高压缩）** | 硬件存储成本可**节省 70% ~ 85%** |
| **亿级多维分析耗时** | 3s ~ 10s（受单核扫描行数限制） | **0.1s ~ 0.5s（向量化并行执行引擎）** | 实时大屏、深度留存漏斗报表 |
| **外部组件依赖** | 极简（仅 PG + Redis + NSQ） | 分布式企业级（ClickHouse 集群） | 运维复杂度考量 |

---

### 开源版 (PostgreSQL) 实测基准数据

测试环境：单节点参考配置（32核 64G 共享服务器，gateway + PostgreSQL 16 / TimescaleDB + Redis 7 + nsqd 1.2.1 同机部署）。

#### 1. 日志写入：并发 - EPS & P95 延迟拐点曲线
...
#### 2. 批量大小 (Batch Size) 策略对比 (恒定 5,000 EPS 压力)
...
|---|---|---:|---:|---:|
| 常用日志流列表 | `/logs/search?limit=50` (项目+时间倒序索引) | **727.8** | **2.8ms** | **4.3ms** |
| 关键词全文检索 | `/logs/search?q=...` (pg_trgm GIN 三元组索引) | **12.9** | **5.8ms** | **41.0ms** |
| 时序聚合统计 | `/logs/trend` (小时级按维聚合) | **2.5** | 8.4s | 9.6s |
| 队列多维留存分析 | `/analytics/retention` (SQL 自连接下推) | **5.0** | 3.0s | 3.0s |

> 详细测试方法、原始时序数据与压测套件见 [`docs/perf/README.md`](docs/perf/README.md)。
